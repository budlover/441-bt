...
void bt_init(bt_config_t *config, int argc, char **argv) {
  ...
  strcpy(config->peer_list_file, "nodes1.map");
  config->argc = argc;
  config->argv = argv;
}
...
void bt_usage() {
  fprintf(stderr, 
	  "usage:  peer [-h] -p <peerfile> -c <chunkfile> -m <maxconn>\n"
...
void bt_help() {
  ...
  fprintf(stderr,
	  ...
	  "         -c <chunkfile>    The list of has chunks\n"
...
void bt_parse_command_line(bt_config_t *config) {
  ...
  while ((c = getopt(argc, argv, _bt_optstring)) != -1) {
    ...
    case 'c': // has chunk file
      strcpy(config->has_chunk_file, optarg);
      break;

    case 'f': // master chunk file
